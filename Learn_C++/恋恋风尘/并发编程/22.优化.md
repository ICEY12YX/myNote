...
两个线程同时push
    ![[Pasted image 20250801174213.png]]
    ![[Pasted image 20250801174224.png]]
    ![[Pasted image 20250801174237.png]]
    上面线程B重新加载old_tail = tail.load(); 有错,因为这个在for外面
    ![[Pasted image 20250801174248.png]]

两个线程同时 increase_external_count
    ![[Pasted image 20250801175013.png]]
    ![[Pasted image 20250801175036.png]]
    之后因为if里比的是data是不是空,所以线程1线程2都可以进,就看谁先竞争到
    ![[Pasted image 20250801175427.png]]
    ![[Pasted image 20250801175458.png]]
    然后线程2再循环一次...
    (后面ai说`counted_node_ptr old_tail = tail.load();`这句应该写在for里面才符合逻辑,不然是错的,好像确实)
    ![[Pasted image 20250801180455.png]]
    之后就线程2单独push了

有些瑕疵
比如线程1,2同时push
1进来 更新old_tail1的external_count为2
2进来 更新old_tail2的external_count为3
然后if判断的时候 线程2可以进 线程1只能`old_tail.ptr->release_ref();`
若此时线程2慢,没执行下去
那线程1会再for循环一次,增加old_tail1的external_count为3
此时线程2还是没执行下去
那线程1还是进不了if
(因为线程2让old_tail.ptr->data=new_data.get(),后面)
会不断for循环(浪费性能) 直到线程2if里面做完
?
总之就是有瑕疵
有点麻烦,可以直接看怎么修改
...
node结构多一个`std::atomic<counted_node_ptr> next;`
...
```
解释
    ![[Pasted image 20250801183350.png]]
    ![[Pasted image 20250801183453.png]]
    ![[Pasted image 20250801183924.png]]
    ![[Pasted image 20250801184216.png]]
    ![[Pasted image 20250801184303.png]]
    ![[Pasted image 20250801185001.png]]
